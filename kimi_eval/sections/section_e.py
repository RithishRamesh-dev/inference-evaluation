...
Spec: 1000 runs. Default: 20 (fast). Pass --eos-runs 1000 for compliance.
"""
import time
...
def run(n_runs: int = 20):
    ...
    empty   = 0
    errors  = 0
    eos_bug = 0   # empty content + finish_reason=stop = premature EOS
    ...
    for i in range(n_runs):
        ...
        if not b and f == "stop":
            eos_bug += 1
        if i > 0 and i % 100 == 0:
            time.sleep(1)   # pace at high n

    total = sum(counts.values())
    if total == 0:
        record("E", "ST-003 EOS statistical test", False, f"all {n_runs} failed")
        return
    ...
    record("E", "ST-001/002 No premature EOS (empty+stop)", eos_bug == 0,
           f"premature EOS events: {eos_bug}/{total}")
